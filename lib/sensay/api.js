...
function getApiVersion() {
  const raw = (process.env.SENSAY_API_VERSION || "").trim();
  const fallback = "2025-03-25";
...
export async function getReplicaHistory(params = {}, userId) {
  ...
  if (!xUserId) throw new Error("Missing userId for Sensay history request");
  ...
  const url = `${BASE_URL}/v1/replicas/${replica}/chat/history${
...
export async function completeReplica(messages, options = {}, userId) {
  ...
  const xUserId = userId;
  if (!xUserId) throw new Error("Missing userId for Sensay experimental completion request");
